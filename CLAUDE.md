...
- **`hydra-anthropic.mjs`** — Streaming client for Anthropic Messages API. Mirrors `hydra-openai.mjs` pattern. Supports extended thinking via `cfg.thinkingBudget` (adds `thinking.budget_tokens` to request body).
...
- **`hydra-nightly.mjs`** — Autonomous overnight task execution. 5-phase pipeline: SCAN (multi-source via tasks-scanner) → DISCOVER (optional AI agent suggests tasks) → PRIORITIZE (dedup + sort + cap) → EXECUTE (per-task: branch isolation, intelligent agent routing via `classifyTask()`/`bestAgentFor()`, model recovery, investigator self-healing, budget-aware handoff) → REPORT (JSON + Markdown). Config-driven via `nightly` section. Supports `--dry-run`, `--no-discovery`, CLI overrides.
