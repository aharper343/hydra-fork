...
export const AFFINITY_PRESETS = {
  'balanced': {
    planning: 0.50, architecture: 0.50, review: 0.50, refactor: 0.50,
    implementation: 0.50, analysis: 0.50, testing: 0.50, security: 0.50,
    research: 0.50, documentation: 0.50,
  },
  'code-focused': {
    planning: 0.40, architecture: 0.35, review: 0.50, refactor: 0.80,
    implementation: 0.85, analysis: 0.45, testing: 0.75, security: 0.30,
    research: 0.20, documentation: 0.40,
  },
  'review-focused': {
    planning: 0.40, architecture: 0.50, review: 0.90, refactor: 0.55,
    implementation: 0.35, analysis: 0.85, testing: 0.60, security: 0.80,
    research: 0.65, documentation: 0.50,
  },
  'research-focused': {
    planning: 0.60, architecture: 0.50, review: 0.55, refactor: 0.30,
    implementation: 0.30, analysis: 0.80, testing: 0.35, security: 0.50,
    research: 0.90, documentation: 0.75,
  },
};
```
        ...
        customAgents: [
          {
            ...
            contextBudget: 16000,
            councilRole: null,
            taskAffinity: { implementation: 0.70, review: 0.40, research: 0.00,
              planning: 0.30, architecture: 0.25, refactor: 0.60, analysis: 0.40,
              testing: 0.55, security: 0.30, documentation: 0.40 },
            enabled: true,
          },
          {
            name: 'test-api-agent',
            ...
            contextBudget: 8000,
            councilRole: null,
            taskAffinity: { implementation: 0.80, review: 0.50, research: 0.00,
              planning: 0.35, architecture: 0.30, refactor: 0.75, analysis: 0.45,
              testing: 0.65, security: 0.25, documentation: 0.45 },
            enabled: true,
          },
        ],
      },
...
```javascript
    ...
    if (Array.isArray(agentsCfg.customAgents)) {
      for (const def of agentsCfg.customAgents) {
        ...
        } catch { /* skip invalid custom agents silently */ }
      }
    }
```
...
async function executeCustomCliAgent(agentName, prompt, opts = {}) {
  ...
  return new Promise((resolve) => {
    ...
    child.stdout.on('data', (d) => { stdout += d; if (onProgress) onProgress(Date.now() - startTime, Math.round(stdout.length / 1024)); });
    ...
    child.on('close', async (code, signal) => {
      ...
      resolve({
        ok: true, output, stdout: output, stderr,
        error: null, errorCategory: null, exitCode: code,
        signal, durationMs, timedOut,
      });
    });
  });
}
...
describe('executeAgent — custom agent routing', () => {
  it('returns custom-cli-disabled for unknown custom CLI agent', async () => {
    ...
    const { registerAgent, _resetRegistry, initAgentRegistry, AGENT_TYPE } = await import('../lib/hydra-agents.mjs');
    ...
    registerAgent('fake-cli', {
      type: AGENT_TYPE.PHYSICAL, customType: 'cli', cli: null,
      invoke: null, enabled: false,
      taskAffinity: {}, contextBudget: 1000, councilRole: null,
    });
