...
const COST_PER_1K = {
  'gpt-5.2-codex':                  { input: 0.002, output: 0.008 },
  'gpt-5.3':                        { input: 0.003, output: 0.012 },
  'gpt-5':                          { input: 0.002, output: 0.008 },
  'o4-mini':                        { input: 0.001, output: 0.004 },
  'claude-opus-4-6':                { input: 0.015, output: 0.075 },
  'claude-sonnet-4-5-20250929':     { input: 0.003, output: 0.015 },
  'claude-haiku-4-5-20251001':      { input: 0.0008, output: 0.004 },
  'gemini-3-pro-preview':           { input: 0.0025, output: 0.015 },
  'gemini-3-flash-preview':         { input: 0.0003, output: 0.001 },
...
function estimateCost(model, usage) {
  ...
  if (!rates) return 0;
  const inputCost = ((usage.prompt_tokens || 0) / 1000) * rates.input;
  const outputCost = ((usage.completion_tokens || 0) / 1000) * rates.output;
...
async function postConciergeEvent(type, payload) {
  ...
  try {
    ...
    // Best-effort, don't block concierge flow
  }
}
...
export async function conciergeSuggest(contextDescription, opts = {}) {
  ...
  const streamCfg = { ...cfg, maxTokens: opts.maxTokens || 80 };
  if (activeProvider) {
    streamCfg.model = activeProvider.model;
  }
