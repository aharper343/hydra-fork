...
function triage(failure, investigatorDiagnosis, recurring, cfg) {
  ...
  if (invDiag === 'fundamental') {
    ...
    // No investigator available — heuristic fallback
    if (failure.timedOut) {
      ...
      severity = 'medium';
      action = 'ticket';
      explanation = failure.error?.slice(0, 200) || 'Unknown failure without investigator';
      rootCause = 'unknown';
    }
  }
...
function buildSignature(failure) {
  ...
  const errorSnippet = (failure.error || '').slice(0, 100).replace(/\s+/g, ' ').trim();
...
async function createFollowUp(failure, diagnosis, type) {
  const title = `[doctor] ${failure.pipeline}: ${diagnosis.explanation.slice(0, 80)}`;
  const notes = [
...
async function addKBEntry(failure, diagnosis) {
  try {
    const { loadKnowledgeBase, saveKnowledgeBase, addEntry } = await import('./hydra-evolve-knowledge.mjs');
...
async function lazyLoadInvestigator() {
  try {
    return await import('./hydra-evolve-investigator.mjs');
  } catch {
    return null;
  }
}
...
export async function scanDoctorLog() {
  ...
  for (const entry of _history) {
    if (entry.action === 'fix' || entry.action === 'ticket') {
      items.push({
        id: `doctor-${entry.ts}-${entry.signature?.slice(0, 20) || 'unknown'}`,
        title: (entry.explanation || 'Unknown issue').slice(0, 100),
        description: [
        ...
        source: 'doctor-log',
        agent: entry.agent || 'codex',
        actionPrompt: buildFixPrompt(entry),
        meta: { entry },
      });
    }
  }
...
/**
 * Execute a single fix action via agent CLI.
...
export async function executeFixAction(item, opts = {}) {
  ...
  try {
    ...
    if (!ok) {
      // Log back to doctor for tracking
      try {
        await diagnose({
          pipeline: 'doctor-fix',
          phase: 'execute',
          agent,
          error: result.error || result.stderr || 'Fix execution failed',
