...
async function main() {
  ...
  if (isPreview) {
    ...
    spinClaude.start();
    const claudeResult = callAgent('claude', claudePrompt, timeoutMs);
    ...
    spinGemini.start();
    const geminiResult = callAgent('gemini', geminiPrompt, timeoutMs);
    ...
    spinCodex.start();
    const codexResult = callAgent('codex', codexPrompt, timeoutMs);
    report.codex = {
      ...codexResult,
