 ...
 * This character-level allowlist prevents obviously dangerous patterns while
 * still supporting every legitimate build/test tool.
 *
...
/**
 * Validate that a verification command is free of shell-injection characters.
 * Returns true when the command is safe to pass to exec/spawn with shell:true.
 * @param {string} command
 * @returns {boolean}
 */
export function isVerificationCommandSafe(command) {
...
export function resolveVerificationPlan(projectRoot, hydraConfig = loadHydraConfig(), signalsOverride = null) {
  ...
  if (rawCommand && lowered !== 'auto') {
    if (!isVerificationCommandSafe(rawCommand)) {
      return {
        enabled: false,
        timeoutMs,
        command: '',
