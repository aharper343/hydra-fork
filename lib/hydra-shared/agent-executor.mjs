...
export function executeAgent(agent, prompt, opts = {}) {
  ...
  return new Promise((resolve) => {
    ...
    if (agent === 'codex') {
      ...
      // Thinking budget for Claude opus/sonnet
      const claudeEffort = effortOverride || getReasoningEffort('claude');
      if (claudeEffort) {
        const claudeModel = modelOverride || getActiveModel('claude');
        const caps = getModelReasoningCaps(claudeModel);
        if (caps.type === 'thinking') {
          const LEGACY_MAP = { high: 'deep', xhigh: 'deep', medium: 'standard', low: 'light' };
          const normalized = LEGACY_MAP[claudeEffort] || claudeEffort;
