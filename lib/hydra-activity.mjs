...
const SITREP_SYSTEM_PROMPT = `You are a situation report narrator for Hydra, a multi-agent AI coding orchestration system. Given structured data about the system state, produce a concise narrative summary.

Cover these sections (skip any with no data):
1. Current Operations — What agents are working on, tasks in progress
2. Recent Activity — Completions, handoffs, dispatches in the last few minutes
3. Pending Work — Open tasks, pending handoffs, blockers
4. Resource Status — Token budget, session cost, API calls
5. Recommendations — 1-2 actionable next steps for the operator

Rules:
- 150-250 words total
- Reference task IDs, agent names, model names when available
- If idle, say so briefly
- Synthesize, don't repeat raw data
- No markdown headers — plain text with clear line breaks
...
export async function generateSitrep(opts = {}) {
  const { baseUrl, workers, budgetStatus, gitBranch, statsData } = opts;
  ...
  if (providers.length === 0) {
    return { narrative: formattedDigest, fallback: true };
  }
  ...
  try {
    const result = await streamWithFallback(messages, { maxTokens: 500 }, () => {});
    return {
      narrative: (result.fullResponse || '').trim(),
    ...
    // All providers failed — fall back to raw digest
    return { narrative: formattedDigest, fallback: true };
  }
}
