...
async function executeAgentWithRetry(agent, prompt, opts = {}) {
  ...
  if (rlCheck.isRateLimit) {
    ...
    notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: result.error || 'rate limited', stderr: result.stderr, stdout: result.output, context: `Rate limited after ${maxRetries} retries` });
  ...
  if (modelCheck.isModelError) {
    ...
    disabledAgents.add(agent);
    notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: modelCheck.errorMessage, stderr: result.stderr, stdout: result.output, context: `Model error: ${modelCheck.failedModel}` });
  ...
  if (isInvestigatorAvailable()) {
    ...
    if (diagnosis.diagnosis === 'fundamental') {
      ...
      disabledAgents.add(agent);
      notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: result.error, stderr: result.stderr, stdout: result.output, context: `Fundamental: ${diagnosis.explanation}` });
    ...
    if (!retry.ok) {
      ...
      notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: retry.error || result.error, stderr: retry.stderr, stdout: retry.output, context: `Investigation + retry failed: ${diagnosis.explanation}` });
    }
  ...
  if (!retry.ok) {
    ...
    notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: retry.error || result.error, stderr: retry.stderr, stdout: retry.output, context: 'Consecutive failures without investigator' });
  }
