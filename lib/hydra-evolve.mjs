...
async function executeAgentWithRetry(agent, prompt, opts = {}) {
  ...
  // If retry also failed, disable this agent for the rest of the session
  if (!retry.ok) {
 ...
 */
async function phaseImplement(plan, branchName, safetyPrompt, { cwd, timeouts, deliberation } = {}) {
  log.phase('IMPLEMENT');

  const improvementDesc = deliberation?.selectedImprovement || '';
