...
function executeAgent(agent, prompt, { cwd, timeoutMs, modelOverride, phaseLabel } = {}) {
  return new Promise((resolve) => {
    ...
    if (agent === 'codex') {
      ...
      // claude — use stdin for prompt to avoid Windows cmd.exe length/escaping issues
      args = ['-p', '-', '--output-format', 'json', '--permission-mode', 'auto-edit'];
      if (modelOverride) args.push('--model', modelOverride);
      cmd = 'claude';
    }
    ...
    // All agents use stdin for prompts (avoids Windows cmd.exe length/escaping issues)
    const spawnOpts = {
      cwd,
      windowsHide: true,
      shell: process.platform === 'win32',
    ...
    // Pipe the prompt to stdin for all agents
...
/**
 * Execute an agent with a single retry on failure.
 * Waits 3s before retrying. Returns the best result.
 */
async function executeAgentWithRetry(agent, prompt, opts = {}) {
  const result = await executeAgent(agent, prompt, opts);
  if (result.ok) return result;

  // Don't retry timeouts (already took too long) or if no time for retry
