...
import { detectModelError, recoverFromModelError, detectRateLimitError, detectUsageLimitError, formatResetTime, calculateBackoff } from './hydra-model-recovery.mjs';
 ...
 * Retries with exponential backoff on HTTP 429 / RESOURCE_EXHAUSTED.
 */
async function executeGeminiDirect(prompt, { timeoutMs, modelOverride, phaseLabel } = {}) {
  ...
  const metricsHandle = recordCallStart('gemini', metricsLabel);
  setAgentActivity('gemini', 'working', phaseLabel || 'Running');
  ...
  try {
    ...
    if (!token) {
      ...
      recordCallError(metricsHandle, 'No Gemini OAuth credentials');
    ...
    if (!projectId) {
      const durationMs = Date.now() - startTime;
      recordCallError(metricsHandle, 'Could not resolve Gemini project ID');
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      if (resp.ok) {
        ...
        recordCallComplete(metricsHandle, { stdout: text, stderr: '' });
      ...
      // Check if this is a rate limit (429 / RESOURCE_EXHAUSTED)
      if (resp.status === 429 || /RESOURCE_EXHAUSTED|QUOTA_EXHAUSTED/i.test(errText)) {
        if (attempt < maxRetries) {
          ...
          const retryAfterMs = serverRetryAfter
            ? parseInt(serverRetryAfter, 10) * 1000
            : null;
          const delay = calculateBackoff(attempt, { baseDelayMs, maxDelayMs, retryAfterMs });
          log.warn(`${label}: 429 rate limited — retrying in ${(delay / 1000).toFixed(0)}s (attempt ${attempt + 1}/${maxRetries})`);
          setAgentActivity('gemini', 'waiting', `Rate limited, retry in ${(delay / 1000).toFixed(0)}s`);
        ...
        setAgentActivity('gemini', 'error', 'Failed');
    ...
    setAgentActivity('gemini', 'error', 'Rate limited');
...
function executeAgent(agent, prompt, { cwd, timeoutMs, modelOverride, phaseLabel } = {}) {
  ...
  const metricsLabel = phaseLabel || 'evolve';
  const metricsHandle = recordCallStart(agent, metricsLabel);
  ...
  return new Promise((resolve) => {
    ...
    const context = phaseLabel ? ` [${phaseLabel}]` : '';
    log.dim(`${label}: started${context}`);
    setAgentActivity(agent, 'working', phaseLabel || 'Running');
    ...
    child.on('error', (err) => {
      ...
      const durationMs = Date.now() - startTime;
      recordCallError(metricsHandle, err.message);
      setAgentActivity(agent, 'error', 'Failed');
      resolve({
        ok: false,
        output: chunks.join(''),
    ...
    child.on('close', (code) => {
      ...
      if (code === 0) {
        ...
        recordCallComplete(metricsHandle, { stdout: chunks.join(''), stderr: stderrChunks.join('') });
        log.dim(`${label}: ${estimatedTokens} est. tokens (${formatDuration(durationMs)})`);
      } else {
        recordCallError(metricsHandle, timedOut ? 'timeout' : `exit code ${code}`);
      }
      setAgentActivity(agent, code === 0 ? 'idle' : 'error', code === 0 ? 'Done' : 'Failed');
      resolve({
        ok: code === 0,
        output: chunks.join(''),
        stderr: stderrChunks.join(''),
...
async function executeAgentWithRetry(agent, prompt, opts = {}) {
  ...
  if (usageCheck.isUsageLimit) {
    ...
    log.warn(`${label}: usage limit reached (resets in ${resetLabel})`);
    disabledAgents.add(agent);
    notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: usageCheck.errorMessage, exitCode: result.exitCode, stderr: result.stderr, stdout: result.output, context: `Usage limit — resets in ${resetLabel}` });
  ...
  if (rlCheck.isRateLimit) {
    ...
    notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: result.error || 'rate limited', exitCode: result.exitCode, stderr: result.stderr, stdout: result.output, context: `Rate limited after ${maxRetries} retries` });
  ...
  if (modelCheck.isModelError) {
    ...
    disabledAgents.add(agent);
    notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: modelCheck.errorMessage, exitCode: result.exitCode, stderr: result.stderr, stdout: result.output, context: `Model error: ${modelCheck.failedModel}` });
  ...
  if (isInvestigatorAvailable()) {
    ...
    if (diagnosis.diagnosis === 'fundamental') {
      ...
      disabledAgents.add(agent);
      notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: result.error, exitCode: result.exitCode, stderr: result.stderr, stdout: result.output, context: `Fundamental: ${diagnosis.explanation}` });
    ...
    if (!retry.ok) {
      ...
      notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: retry.error || result.error, exitCode: retry.exitCode ?? result.exitCode, stderr: retry.stderr, stdout: retry.output, context: `Investigation + retry failed: ${diagnosis.explanation}` });
    }
  ...
  if (!retry.ok) {
    ...
    notifyDoctor({ pipeline: 'evolve', phase: opts.phaseLabel || 'agent', agent, error: retry.error || result.error, exitCode: retry.exitCode ?? result.exitCode, stderr: retry.stderr, stdout: retry.output, context: 'Consecutive failures without investigator' });
  }
 ...
 */
async function phaseImplement(plan, branchName, safetyPrompt, { cwd, timeouts, investigatorPreamble, deliberation }) {
  log.phase('IMPLEMENT');
