...
function executeAgent(agent, prompt, { cwd, timeoutMs, modelOverride } = {}) {
  return new Promise((resolve) => {
    ...
    if (agent === 'codex') {
      args = ['exec', prompt, '-s', 'auto-edit', '-C', cwd];
    ...
    const spawnOpts = {
      ...
      windowsHide: true,
      shell: process.platform === 'win32',
      stdio: agent === 'gemini' ? ['pipe', 'pipe', 'pipe'] : ['ignore', 'pipe', 'pipe'],
    };

    const child = spawn(cmd, args, spawnOpts);

    // For gemini, pipe the prompt to stdin
    if (agent === 'gemini') {
...
Respond with JSON:
{
  ...
  log.dim(`Synthesis: ${synthResult.ok ? 'OK' : 'FAIL'}`);
...
Respond with JSON:
{
  ...
  log.dim(`Critique: ${critiqueResult.ok ? 'OK' : 'FAIL'}`);
...
Respond with JSON:
{
  ...
  log.dim(`Priority: ${priorityResult.ok ? 'OK' : 'FAIL'}`);
...
Respond with JSON:
{
  ...
  log.dim(`Plan: ${planResult.ok ? 'OK' : 'FAIL'}`);
...
async function main() {
  ...
  if (currentBranch !== baseBranch) {
    log.info(`Switching from '${currentBranch}' to '${baseBranch}'...`);
    spawnSync('git', ['checkout', baseBranch], { cwd: projectRoot, stdio: 'pipe' });
  }

  if (!isCleanWorkingTree(projectRoot)) {
    log.warn('Working tree is not clean.');
    const answer = await new Promise((resolve) => {
      process.stdout.write(`  ${pc.yellow('?')} Auto-commit all changes before evolve? ${pc.dim('[Y/n]')} `);
      ...
      process.stdin.once('data', (buf) => {
        ...
        process.stdout.write(ch === 'n' ? 'n\n' : 'y\n');
        resolve(ch || 'y');
      });
    });
    if (answer === 'n') {
      log.error('Aborted — commit or stash changes first.');
      process.exit(1);
    }
    const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    spawnSync('git', ['add', '-A'], { cwd: projectRoot, stdio: 'pipe' });
    spawnSync('git', ['commit', '-m', `chore: auto-commit before evolve session ${ts}`], { cwd: projectRoot, stdio: 'pipe' });
  ...
  console.log('');
  console.log(pc.bold('=== Evolve Session Complete ==='));
  console.log(`  Rounds: ${roundResults.length}/${maxRounds}`);
  console.log(`  Results: ${pc.green(approved + ' approved')}, ${pc.yellow(revised + ' revised')}, ${pc.red(rejected + ' rejected')}`);
  console.log(`  Tokens: ~${totalTokens.toLocaleString()} consumed`);
  console.log(`  Duration: ${formatDuration(finishedAt - startedAt)}`);
  console.log(`  Knowledge: ${kbDelta.added} new entries (${kbDelta.total} total)`);
