 ...
 * Returns { ok, method, conflicts[] }
 */
function smartMerge(cwd, branchName, baseBranch) {
  // 1. Check if base has diverged
  const isAncestor = git(['merge-base', '--is-ancestor', baseBranch, branchName], cwd);
  const baseDiverged = isAncestor.status !== 0;

  if (baseDiverged) {
    log.info(`Base branch '${baseBranch}' has diverged — attempting rebase...`);

    // 2. Try rebase
    const rebase = git(['rebase', baseBranch, branchName], cwd);
    if (rebase.status !== 0) {
      // Conflict during rebase — abort and fall through
      git(['rebase', '--abort'], cwd);
      log.warn('Rebase had conflicts — falling back to merge...');
    } else {
      log.ok(`Rebased ${branchName} onto ${baseBranch}`);
      // After successful rebase, we're on branchName. Checkout base and merge (fast-forward).
      checkoutBranch(cwd, baseBranch);
      const ff = git(['merge', branchName, '--ff-only'], cwd);
      if (ff.status === 0) {
        return { ok: true, method: 'rebase+ff', conflicts: [] };
      }
      // Shouldn't happen after clean rebase, but fall through
    }
  }

  // 3. Regular merge attempt
  checkoutBranch(cwd, baseBranch);
  ...
  // 4. Merge failed — extract conflicting files
...
async function executeGeminiDirect(prompt, { timeoutMs, modelOverride, phaseLabel } = {}) {
  ...
  try {
    ...
    if (!token) {
      return { ok: false, output: '', stderr: '', error: 'No Gemini OAuth credentials (~/.gemini/oauth_creds.json)', durationMs: Date.now() - startTime, timedOut: false };
    }

    const projectId = await getGeminiProjectId(token);
    if (!projectId) {
      return { ok: false, output: '', stderr: '', error: 'Could not resolve Gemini project ID', durationMs: Date.now() - startTime, timedOut: false };
    }
    ...
    if (!resp.ok) {
      ...
      return { ok: false, output: '', stderr: errText, error: `Gemini API ${resp.status}`, durationMs: Date.now() - startTime, timedOut: false };
    }
    ...
    return { ok: true, output: text, stderr: '', durationMs: Date.now() - startTime, timedOut: false };
  } catch (err) {
...
Respond with a JSON object:
{
  ...
  // Dispatch both agents in parallel (with retry on failure)
  ...
  for (const [name, result] of [['Claude', claudeResult], ['Gemini', geminiResult]]) {
    if (!result.ok) {
  ...
  for (const [name, result, data] of [['Claude', claudeResult, claudeData], ['Gemini', geminiResult, geminiData]]) {
    if (result.ok && !data) {
...
async function main() {
  ...
  for (let round = startRound; round <= maxRounds; round++) {
    ...
    try {
      ...
      if (deliberation.selectedImprovement === 'No improvement selected' || deliberation.selectedImprovement.length < 10) {
...
${safetyPrompt}`;
      ...
      if (decision.verdict === 'approve' && didModifyHydraCode(projectRoot, branchName, baseBranch)) {
        ...
        const mergeResult = smartMerge(projectRoot, branchName, baseBranch);
        if (!mergeResult.ok) {
  ...
  for (const r of roundResults) {
    if (r.verdict === 'approve' && r.branchName && !r.merged) {
      ...
      checkoutBranch(projectRoot, baseBranch);
      const result = smartMerge(projectRoot, r.branchName, baseBranch);
      r.merged = result.ok;
