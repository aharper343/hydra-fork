...
import { getAgent, AGENTS, KNOWN_OWNERS, AGENT_NAMES, classifyTask, TASK_TYPES, getModelSummary } from './hydra-agents.mjs';
...
function startDaemon(options) {
  ...
  function runVerification(taskId, plan) {
    ...
    exec(plan.command, { cwd: config.projectRoot, timeout: plan.timeoutMs, encoding: 'utf8' }, (error, stdout, stderr) => {
      if (error) {
  ...
  // Stale task reaper: mark in_progress tasks as stale after 30 minutes of inactivity
  ...
  function markStaleTasks() {
    try {
      ...
      for (const task of state.tasks) {
        ...
        // Use latest checkpoint savedAt if available, otherwise updatedAt
