    ...
    try {
      ...
      if (line === ':resume') {
        try {
          ...
          // Launch terminals for all agents with work
          const launchList = [...agentsToLaunch].filter((a) => agents.includes(a));
          if (launchList.length > 0) {
            console.log('');
            launchAgentTerminals(launchList, baseUrl);
          }
      ...
      if (mode === 'auto' || mode === 'smart') {
        ...
        setDispatchContext({
          promptSummary: topic || short(line, 30),
          topic,
          tier: classification.tier,
        });
        ...
        if (auto.escalatedToCouncil) {
          ...
          if (handoffAgents.length > 0) launchAgentTerminals(handoffAgents, baseUrl);
        } else {
