...
const KNOWN_COMMANDS = [
  ...
  ':confirm', ':shutdown', ':quit', ':exit', ':sync',
];
...
async function interactiveLoop({
  ...
  function buildConciergePrompt() {
    ...
    return `${ACCENT('hydra')}${pc.blue('\u2B22')}${modelSuffix}${DIM('>')} `;
  }
  ...
  function showConciergeWelcome() {
    ...
    console.log(`  ${DIM('Chat naturally or ask questions about your project.')}`);
    console.log(`  ${DIM('Prefix')} ${ACCENT('!')} ${DIM('to force dispatch to agents.')}`);
    console.log(`  ${DIM(':chat off to disable, :chat stats for usage.')}`);
    console.log('');
  }
