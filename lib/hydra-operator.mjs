...
import { checkUsage, renderUsageDashboard, renderUsageBar, getContingencyOptions, executeContingency } from './hydra-usage.mjs';
...
function printWelcome(baseUrl) {
  ...
  try {
    ...
    if (usage.level !== 'unknown') {
      console.log(label('Usage', progressBar(usage.percent || 0, 20)));
    }
  } catch { /* skip */ }
...
async function runCouncilPrompt({ baseUrl, promptText, rounds = 2, preview = false }) {
  ...
  const result = await spawnAsync('node', args, { cwd: config.projectRoot });
...
async function runCouncilJson({ baseUrl, promptText, rounds = 1, preview = false, publish = false }) {
  const councilScript = path.join(HYDRA_ROOT, 'lib', 'hydra-council.mjs');
  const args = [
    councilScript,
    `prompt=${promptText}`,
    ...
    try {
      ...
      if (mode === 'auto') {
        ...
        const spinner = classification.tier === 'simple'
          ? createSpinner(`Fast-path \u2192 ${classification.suggestedAgent}`).start()
          : createSpinner(`Classifying as ${classification.tier}\u2026 running ${classification.tier === 'complex' ? 'council' : 'mini-round triage'}`).start();
        let auto;
        try {
        ...
        const councilSpinner = createSpinner('Running council deliberation\u2026').start();
        const council = await runCouncilPrompt({
          baseUrl,
          promptText: line,
          rounds: councilRounds,
        ...
        const handoffSpinner = createSpinner('Dispatching to agents\u2026').start();
