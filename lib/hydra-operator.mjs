...
import { getAgent, AGENT_NAMES, getActiveModel, getModelSummary, setActiveModel, getMode, setMode, resetAgentModel, getVerifier, listAgents, resolvePhysicalAgent, AGENT_TYPE, registerAgent, unregisterAgent, formatEffortDisplay } from './hydra-agents.mjs';
...
import { getSessionUsage, getMetricsSummary, estimateFlowDuration } from './hydra-metrics.mjs';
import { resolveProject, HYDRA_ROOT, loadHydraConfig, getRoleConfig, getRecentProjects } from './hydra-config.mjs';
    ...
    try {
      ...
      if (line === ':model' || line.startsWith(':model ')) {
        ...
        if (modelArgs) {
          ...
          console.log(DIM(`  Override:  :model codex=gpt-5.3-codex`));
      ...
      if (line === ':clear' || line.startsWith(':clear ')) {
        ...
        try {
          ...
          if (what !== 'all' && what !== 'handoffs' && what !== 'tasks') {
            console.log(`  ${ERROR('Unknown target:')} ${what}`);
            console.log(`  ${DIM('Usage: :clear [all|tasks|handoffs]')}`);
            rl.prompt();
            return;
          }
      ...
      if (line === ':tasks' || line.startsWith(':tasks ')) {
        ...
        if (tasksArg === 'run' || tasksArg.startsWith('run ')) {
          ...
          const child = spawn('node', tasksArgs, {
            cwd,
            stdio: 'inherit',
        ...
        if (tasksArg === 'review') {
          ...
          const child = spawn('node', [reviewScript, 'review'], {
            cwd,
            stdio: 'inherit',
            shell: process.platform === 'win32',
          });
        ...
        if (tasksArg === 'clean') {
          const reviewScript = path.join(HYDRA_ROOT, 'lib', 'hydra-tasks-review.mjs');
          const child = spawn('node', [reviewScript, 'clean'], {
            cwd: config.projectRoot,
            stdio: 'inherit',
            shell: process.platform === 'win32',
          });
        ...
        try {
          ...
          if (!doctorArg || doctorArg === 'stats') {
            ...
            if (!errorText) {
              ...
              try {
                const inv = await import('./hydra-evolve-investigator.mjs');
            ...
            console.log(`  ${DIM('Usage: :doctor [stats|log|diagnose <text>]')}`);
          }
        } catch (e) {
          console.log(`  ${ERROR(e.message)}`);
        }
      ...
      if (line === ':kb' || line.startsWith(':kb ')) {
        ...
        try {
          const { loadKnowledgeBase, searchEntries, getStats } = await import('./hydra-evolve-knowledge.mjs');
      ...
      if (line === ':nightly' || line.startsWith(':nightly ')) {
        ...
        rl.pause();
        destroyStatusBar();
        const child = spawn('node', nightlyArgs, {
          cwd,
          stdio: 'inherit',
          shell: process.platform === 'win32',
        });
      ...
      // ── Concierge Intercept ────────────────────────────────────────────
        ...
        // Start paste buffering: wait briefly for more lines before processing
        if (!_isPastedInput) {
          _pasteBuffer = [line];
          _pasteTimer = setTimeout(() => {
            const fullInput = _pasteBuffer.join('\n').trim();
      ...
      if (mode === 'auto' || mode === 'smart') {
        ...
        const spinner = classification.tier === 'simple'
          ...
          : createSpinner(`${smartLabel}Running ${classification.tier === 'complex' ? 'council deliberation' : 'mini-round triage'}`, { estimatedMs: estMs, style: classification.tier === 'complex' ? 'orbital' : 'solar' }).start();
