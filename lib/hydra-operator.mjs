...
import { resolveProject, HYDRA_ROOT, loadHydraConfig, saveHydraConfig, getRoleConfig, getRecentProjects, getProviderPresets } from './hydra-config.mjs';
...
async function runAutoPrompt({ baseUrl, from, agents, promptText, miniRounds, councilRounds, preview, onProgress = null }) {
  ...
  const { text: _gatedText, classification } = await gateIntent(promptText, {
...
async function runAutoPromptLegacy({ baseUrl, from, agents, promptText, miniRounds, councilRounds, preview, onProgress, classification }) {
  ...
  const { text: _gatedText2, classification: _gatedClassification } = await gateIntent(promptText, {
    enabled: _intentCfg2.enabled !== false,
    confidenceThreshold: _intentCfg2.confidenceThreshold ?? 0.55,
  });
  const effectivePrompt = _gatedText2;
  // Use caller-supplied classification if provided (from runAutoPrompt gate), otherwise use gate result
  if (!classification) classification = _gatedClassification;
