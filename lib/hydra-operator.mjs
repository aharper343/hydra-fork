...
function startAgentWorker(agent, baseUrl, { rl } = {}) {
  ...
  worker.on('task:complete', ({ agent: a, taskId, status, durationMs, outputSummary }) => {
    ...
    pushActivity(status === 'error' ? 'error' : 'completion', annotateCompletion({
      agent: a, taskId, title: '', durationMs, outputSummary, status,
    ...
    const icon = SUCCESS('\u2713');
    const sparkle = '\u2728'; // ✨
    const msg = `  ${icon} ${sparkle} ${colorAgent(a)} completed ${pc.white(taskId)}${elapsed ? ` ${DIM(`in ${elapsed}`)}` : ''}`;
  ...
  worker.on('task:error', ({ agent: a, taskId, error }) => {
    pushActivity('error', annotateCompletion({
      agent: a, taskId, title: '', status: 'error', outputSummary: error,
    }), { agent: a, taskId });
...
const KNOWN_COMMANDS = [
  ...
  ':github', ':pr', ':confirm', ':shutdown', ':quit', ':exit', ':sync',
];
  ...
  rl.on('line', async (lineRaw) => {
    ...
    try {
      ...
      // ── Concierge Intercept ────────────────────────────────────────────
        ...
        const modelLbl = getConciergeModelLabel();
        const spinner = createSpinner(`${pc.blue('\u2B22')} ${DIM(modelLbl)} thinking...`, { style: 'pulse' });
        spinner.start();
