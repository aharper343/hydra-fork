      ...
      if (mode === 'auto' || mode === 'smart') {
        ...
        const onProgress = (evt) => {
          if (evt.action === 'start') {
            ...
            spinner.update(`${smartLabel}${prefix}: ${evt.agent} ${verb}...  [${evt.step}/${evt.totalSteps}]`);
            setAgentActivity(evt.agent, 'working', `${verb}...`, { phase: evt.phase, step: `${evt.step}/${evt.totalSteps}` });
            drawStatusBar();
          }
        };
        let auto;
