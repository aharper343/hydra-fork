...
import { getProjectContext, buildAgentContext } from './hydra-context.mjs';
...
import { resolveProject, HYDRA_ROOT, HYDRA_RUNTIME_ROOT, loadHydraConfig, saveHydraConfig, getRoleConfig, getRecentProjects, getProviderPresets, diffConfig } from './hydra-config.mjs';
...
import { gateIntent } from './hydra-intent-gate.mjs';
...
async function runAutoPrompt({ baseUrl, from, agents, promptText, miniRounds, councilRounds, preview, onProgress = null }) {
  ...
  const _intentCfg = (() => {
    try { const c = loadHydraConfig(); return c.routing?.intentGate ?? {}; } catch { return {}; }
  })();
  let _gatedText, classification;
  try {
    ({ text: _gatedText, classification } = await gateIntent(promptText, {
      enabled: _intentCfg.enabled !== false,
      confidenceThreshold: _intentCfg.confidenceThreshold ?? 0.55,
    }));
  } catch {
  ...
  if (routingConfig.useLegacyTriage && routeStrategy !== 'single') {
    return runAutoPromptLegacy({ baseUrl, from, agents, promptText: effectivePrompt, miniRounds, councilRounds, preview, onProgress, classification });
  }
  ...
  if (routeStrategy === 'single') {
    ...
    const published = await publishFastPathDelegation({ baseUrl, from, promptText: effectivePrompt, classification, agents });
    return {
      mode: 'fast-path',
      recommended: 'handoff',
  ...
  if (routeStrategy === 'tandem') {
    ...
    const published = await publishTandemDelegation({ baseUrl, from, promptText: effectivePrompt, classification, agents });
  ...
  if (shouldCrossVerify(classification) && council.stdout) {
    verification = await runCrossVerification('claude', council.stdout.trim(), effectivePrompt, spec?.specContent);
  }
...
async function runAutoPromptLegacy({ baseUrl, from, agents, promptText, miniRounds, councilRounds, preview, onProgress, classification }) {
  ...
  if (classification.tier === 'complex') {
    try { spec = await generateSpec(effectivePrompt, null, { cwd: config.projectRoot }); } catch { /* non-critical */ }
  }

  if (recommended === 'council' || classification.tier === 'complex') {
    const council = await runCouncilPrompt({ baseUrl, promptText: effectivePrompt, rounds: councilRounds, preview: false, onProgress, agents });
  ...
  const published = await publishMiniRoundDelegation({ baseUrl, from, agents, promptText: effectivePrompt, report: triage.report });
...
const COMMAND_HELP = {
  ...
  ':agents':   { usage: [':agents', ':agents list [virtual|physical|all]', ':agents info <name>', ':agents enable <name>', ':agents disable <name>', ':agents add'], desc: 'Agent registry management' },
  ':doctor':   { usage: [':doctor', ':doctor log', ':doctor fix', ':doctor config', ':doctor diagnose <text>'], desc: 'Diagnostics & self-healing' },
    ...
    try {
      ...
      if (line === ':tasks' || line.startsWith(':tasks ')) {
        ...
        if (tasksArg === 'review') {
          ...
          child.on('close', async () => {
            ...
            try {
              const { state } = await request('GET', baseUrl, '/state');
              const conflictTasks = (state.tasks || []).filter((t) => t.worktreeConflict);
              if (conflictTasks.length > 0) {
                console.log('');
                console.log(sectionHeader('Conflict Worktrees'));
                console.log(`  ${WARNING('\u26A0')}  ${conflictTasks.length} task worktree${conflictTasks.length > 1 ? 's' : ''} have merge conflicts and were preserved for manual resolution:`);
                console.log('');
                for (const t of conflictTasks) {
                  const worktreePath = t.worktreePath || `<unknown>`;
                  ...
                  console.log(`    ${DIM('Branch:')}   ${t.worktreeBranch || `hydra/task/${t.id}`}`);
                  console.log('');
                  console.log(`    ${DIM('Inspect:')}  git worktree list`);
                  console.log(`    ${DIM('Diff:')}     git diff ${t.worktreeBranch || `hydra/task/${t.id}`}`);
                  console.log('');
                }
                console.log(`  ${DIM('To discard a conflict worktree, use :cleanup — stale worktrees appear after 24h.')}`);
                console.log(`  ${DIM('To resolve manually: edit the conflicted files, commit, then run :cleanup.')}`);
              }
            } catch { /* daemon may be unavailable — skip silently */ }
            initStatusBar(agents);
            rl.resume();
            rl.prompt();
          });
      ...
      if (line === ':agents' || line.startsWith(':agents ')) {
        ...
        if (!agentSubCmd) {
          ...
          // :agents add — interactive wizard with optional provider preset
          const presets = getProviderPresets();

          console.log('');
          console.log(sectionHeader('Add Custom Agent'));
          ...
          if (presets.length > 0) {
            console.log(`  Use a provider preset?`);
            for (let i = 0; i < presets.length; i++) {
              console.log(`    ${pc.bold(`[${i + 1}]`)} ${presets[i].label} ${DIM('—')} ${DIM(presets[i].description)}`);
            }
            console.log(`    ${pc.bold('[0]')} No preset — enter manually`);
            console.log('');

            const choice = await new Promise(resolve => {
              rl.question('  Choice [0]: ', ans => resolve(ans.trim() || '0'));
            });
            ...
            if (idx > 0 && idx <= presets.length) {
              ...
              const keySet = process.env[preset.envKey];
              const envMsg = keySet
                ? SUCCESS(`\u2713 ${preset.envKey} is set`)
                : ERROR(`\u2717 ${preset.envKey} not set — add it before using this agent`);
              console.log(`  ${envMsg}`);
              console.log('');
            }
          }

          const nameSuggestion = presetLabel ? presetLabel.split(' ')[0].toLowerCase() : '';
          const name = await new Promise(resolve => {
            rl.question(`  Agent name${nameSuggestion ? ` [${nameSuggestion}]` : ''}: `, ans => {
              resolve(ans.trim() || nameSuggestion);
            });
          });
          if (!name) { console.log(`  ${ERROR('Agent name required.')}`); }

          else {
            const url = await new Promise(resolve => {
              rl.question(`  Base URL${baseUrl ? ` [${baseUrl}]` : ''}: `, ans => {
                resolve(ans.trim() || baseUrl);
              });
            });

            if (!url) { console.log(`  ${ERROR('Base URL required.')}`); }
            else {
              const model = await new Promise(resolve => {
                rl.question(`  Model${defaultModel ? ` [${defaultModel}]` : ''}: `, ans => {
                  resolve(ans.trim() || defaultModel);
                });
              });

              if (!model) { console.log(`  ${ERROR('Model required.')}`); }
              else {
                // Write to hydra.config.json
                try {
                  const configPath = path.join(process.cwd(), 'hydra.config.json');
                  let userConfig = {};
                  try { userConfig = JSON.parse(fs.readFileSync(configPath, 'utf8')); } catch { /* fresh */ }
                  if (!userConfig.agents) userConfig.agents = {};
                  if (!Array.isArray(userConfig.agents.customAgents)) userConfig.agents.customAgents = [];

                  if (userConfig.agents.customAgents.find(a => a.name === name)) {
                    console.log(`  ${ERROR('Agent already exists:')} ${name}. Use :agents remove first.`);
                  } else {
                    userConfig.agents.customAgents.push({ name, type: 'api', baseUrl: url, model, enabled: true });
                    fs.writeFileSync(configPath, JSON.stringify(userConfig, null, 2) + '\n');
                    console.log(`  ${SUCCESS('\u2713')} Added agent ${pc.bold(name)} \u2192 ${DIM(url)} / ${pc.white(model)}`);
                    console.log(`  ${DIM('Restart or :reload to activate.')}`);
                  }
                } catch (err) {
                  console.log(`  ${ERROR('Failed to write config:')} ${err.message}`);
                }
              }
            }
          }
          ...
          console.log(`  ${DIM('Usage: :agents [info|list|enable|disable|add] [name]')}`);
        }
          ...
          if (!doctorArg || doctorArg === 'stats') {
            ...
            try {
              ...
              try {
                ...
                console.log(`  ${WARNING('Could not read hydra.config.json — using empty config for comparison')}`);
              }
              ...
              if (stale.length === 0) {
                ...
                console.log(`  ${ERROR('✗')} ${pc.bold('Stale keys')} ${DIM('(in your config but not in DEFAULT_CONFIG — may be from an old version):')}`);
                for (const item of stale) {
                  const preview = JSON.stringify(item.userValue);
                  const truncated = preview.length > 60 ? preview.slice(0, 57) + '...' : preview;
                  console.log(`    ${ERROR('•')} ${pc.white(item.path.padEnd(32))} ${DIM('user: ' + truncated)}`);
                }
                console.log(`  ${DIM('  → Consider removing these keys or updating your config.')}`);
              }
              console.log('');
              ...
              if (typeMismatches.length === 0) {
                ...
                console.log(`  ${ERROR('✗')} ${pc.bold('Type mismatches')} ${DIM('(key exists in both but types differ):')}`);
                for (const item of typeMismatches) {
                  console.log(`    ${ERROR('•')} ${pc.white(item.path.padEnd(32))} expected: ${WARNING(item.expectedType.padEnd(8))}  got: ${ERROR(item.gotType)}`);
                }
                console.log(`  ${DIM('  → These may cause unexpected behaviour — check your config values.')}`);
              }
              console.log('');
              ...
              if (missing.length === 0) {
                ...
                console.log(`  ${WARNING('⚠')} ${pc.bold('Missing keys')} ${DIM('(in DEFAULT_CONFIG but not in your file — defaults will be used, no action needed):')}`);
                for (const item of missing) {
                  const preview = JSON.stringify(item.defaultValue);
                  const truncated = preview.length > 60 ? preview.slice(0, 57) + '...' : preview;
                  console.log(`    ${DIM('•')} ${pc.white(item.path.padEnd(32))} ${DIM('default: ' + truncated)}`);
                }
                console.log(`  ${DIM('  → These are informational only. Hydra fills them in automatically.')}`);
              }
              console.log('');
            } catch (e) {
              console.log(`  ${ERROR(e.message)}`);
      ...
      if (line === ':cleanup') {
        try {
          ...
          const { scanArchivableTasks, scanOldHandoffs, scanStaleBranches,
                  ...
                  scanOldArtifacts, scanStaleTaskWorktrees, enrichCleanupWithSitrep,
