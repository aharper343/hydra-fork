...
function printWelcome(baseUrl) {
  ...
  try {
    ...
    for (const [agent, info] of Object.entries(models)) {
      ...
      const colorFn = { claude: pc.red, gemini: pc.cyan, codex: pc.green }[agent] || pc.white;
    ...
    try {
      ...
      if (mode === 'auto') {
        ...
        const spinner = classification.tier === 'simple'
          ? createSpinner(`Fast-path \u2192 ${classification.suggestedAgent}`, { estimatedMs: estMs }).start()
          : createSpinner(`Running ${classification.tier === 'complex' ? 'council deliberation' : 'mini-round triage'}`, { estimatedMs: estMs }).start();
