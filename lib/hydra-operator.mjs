...
/**
 * Ensure the terminal window is at least minRows x minCols.
 * Uses VT100 resize escape on modern terminals, falls back to `mode con` on Windows.
 */
function ensureTerminalSize(minRows = 35, minCols = 120) {
  ...
  const targetCols = Math.max(currentCols, minCols);

  // Try VT100 window resize (xterm-compatible, works in Windows Terminal)
  // ESC[8;rows;colst — resize window to rows x cols characters
  process.stdout.write(`\x1b[8;${targetRows};${targetCols}t`);

  // Fallback for classic Windows consoles: mode con sets buffer/window size
  if (process.platform === 'win32') {
    try {
      spawnSync('mode', ['con:', `cols=${targetCols}`, `lines=${targetRows}`], {
        windowsHide: true,
        stdio: 'ignore',
        timeout: 2_000,
      });
    } catch { /* ignore — VT100 escape likely already worked */ }
  }
}
...
function launchAgentTerminals(agentNames, baseUrl) {
  ...
  for (const agent of agentNames) {
    ...
    const icon = { claude: '\u2666', gemini: '\u2726', codex: '\u25B6' }[agent] || '\u25CF';
...
async function interactiveLoop({
  ...
  // Resize terminal before printing anything so the splash is fully visible
  ensureTerminalSize(35, 120);
  ...
  rl.on('line', async (lineRaw) => {
    ...
    try {
      ...
      // Catch unrecognized : commands so they don't get dispatched as prompts
      if (line.startsWith(':')) {
      ...
      if (mode === 'auto' || mode === 'smart') {
        ...
        const councilTopic = extractTopic(line);
        ...
        setDispatchContext({
          promptSummary: councilTopic || short(line, 30),
          topic: councilTopic,
          tier: 'complex',
        });
        ...
        const handoffTopic = extractTopic(line);
        ...
        for (const item of records) {
          ...
          const title = handoffTopic || short(line, 40);
          if (agentName && title) {
