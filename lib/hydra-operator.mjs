...
import { getAgent, AGENT_NAMES, getActiveModel, getModelSummary, setActiveModel, getMode, setMode, resetAgentModel, getVerifier, listAgents, resolvePhysicalAgent, AGENT_TYPE, registerAgent, unregisterAgent } from './hydra-agents.mjs';
...
const KNOWN_COMMANDS = [
  ...
  ':clear', ':archive', ':events', ':workers', ':watch', ':chat', ':evolve',
  ':github', ':pr', ':forge', ':confirm', ':shutdown', ':quit', ':exit', ':sync',
];
    ...
    try {
      ...
      if (line === ':sitrep') {
        ...
        try {
          ...
          if (result.fallback) {
            console.log(`\n  ${DIM('(no AI provider available — showing raw digest)')}\n`);
      ...
      if (line === ':roles') {
        ...
        for (const [role, rc] of Object.entries(roles)) {
          ...
          const effortStr = rc.reasoningEffort ? pc.yellow(` [${rc.reasoningEffort}]`) : '';
