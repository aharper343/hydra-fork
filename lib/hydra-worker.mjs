...
import { getAgent, AGENTS } from './hydra-agents.mjs';
...
function buildHeadlessCommand(agent, prompt, opts = {}) {
  ...
  switch (agent) {
    ...
    case 'gemini':
      return {
        cmd: 'gemini',
        args: ['-p', '-', '--approval-mode', permissionMode, '-o', 'json'],
        useStdin: true,
        prompt,
      };
    case 'codex': {
      const sandbox = permissionMode === 'full-auto' ? 'full-auto' : 'auto-edit';
...
export class AgentWorker extends EventEmitter {
  ...
  async _workLoop() {
    while (!this._stopped) {
      try {
        ...
        const result = await this._executeAgent(prompt);
  ...
  _buildTaskPrompt(task) {
    ...
    const rolePrompt = agentConfig?.rolePrompt || '';
  ...
  _executeAgent(prompt) {
    return new Promise((resolve) => {
      const agentConfig = getAgent(this.agent);
      const timeout = agentConfig?.timeout || 7 * 60 * 1000;

      const { cmd, args, useStdin, prompt: stdinPrompt } = buildHeadlessCommand(this.agent, prompt, {
