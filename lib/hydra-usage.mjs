...
export function checkUsage(options = {}) {
  ...
  // Sliding window budget check — use tighter constraint
  ...
  if (window.tightest && window.tightest.level !== 'unknown') {
    ...
    const dailyLevel = LEVEL_ORDER[level] ?? -1;
    const windowLevel = LEVEL_ORDER[window.tightest.level] ?? -1;
    if (windowLevel > dailyLevel) {
...
function renderUsageDashboard(usage) {
  ...
  lines.push('');
  lines.push(`${b(pc.magenta('HYDRA'))} ${d('|')} ${d('Claude Code Token Burn')}`);
  ...
  if (usage.level === 'unknown') {
    ...
    // Usage bar
    lines.push('');
    lines.push(`  ${d('CLI Burn:')} ${renderUsageBar(usage.percent)}`);
