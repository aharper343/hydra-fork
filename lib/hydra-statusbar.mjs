...
import { metricsEmitter, getSessionUsage } from './hydra-metrics.mjs';
import {
  AGENT_ICONS,
  AGENT_COLORS,
  HEALTH_ICONS,
...
function buildContextLine(cols) {
  ...
  const MODE_ICONS = { smart: '\u26A1', auto: '\u21BB', handoff: '\u2192', council: '\u2694', dispatch: '\u2699' };
  ...
  if (lastPart) leftParts.push(DIM(lastPart));
  const leftText = ` ${leftParts.join(DIM('  \u2502  '))}`;

  // Right: session cost + token gauge
  let rightText = '';
  try {
    const now = Date.now();
    ...
    if (usage && usage.level !== 'unknown' && usage.percent !== undefined) {
      const gauge = compactProgressBar(usage.percent, 12);
      rightText = `${costStr ? DIM(costStr) + '  ' : ''}${gauge}`;
    } else if (costStr) {
...
function buildStatusBar() {
  ...
  // Line 3: agent segments joined by | (enhanced with metadata)
  const segments = [];
  for (const agent of registeredAgents) {
    ...
    // Build rich action text from metadata
    let actionText = state.action || state.status || 'Inactive';
    if (state.status === 'working') {
      const parts = [];
      if (state.model) parts.push(state.model);
      if (state.taskTitle) {
        parts.push(state.taskTitle.slice(0, 25));
      } else if (state.action && !state.action.startsWith('Calling ')) {
        parts.push(state.action.slice(0, 25));
      }
      if (state.step) parts.push(`[${state.step}]`);
      if (state.phase) parts.push(`(${state.phase})`);
      if (parts.length > 0) {
        actionText = parts.join(': ');
        if (!state.model && state.action?.startsWith('Calling ')) {
          actionText = state.action;
        }
      }
    }

    const actionWithElapsed = `${actionText}${elapsed}`;
