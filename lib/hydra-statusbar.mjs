  ...
  switch (payload.event) {
    case 'handoff_ack': {
      ...
      if (agentList.has(agent)) {
        ...
        pushTickerEvent(`${agent} ack'd ${payload.handoffId || '?'}`, 'handoff');
    ...
    case 'task_update': {
      ...
      if (status === 'done') {
        ...
        pushTickerEvent(`${payload.taskId || '?'} done`, 'done');
        ...
        pushTickerEvent(`${payload.taskId || '?'} blocked`, 'blocked');
      }
      break;
    }
    case 'verify': {
    ...
    case 'task_stale': {
      ...
      pushTickerEvent(`${payload.taskId || '?'} stale (${owner})`, 'stale');
      break;
    }
    default:
      break;
