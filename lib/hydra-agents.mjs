...
/**
 * Get the reasoning effort override for an agent (or null for default).
 */
...
/**
 * Set reasoning effort for an agent. Pass null to clear.
 */
 ...
 * Includes reasoning effort flags where supported.
 */
export function getModelFlags(agentName) {
  ...
  if (modelId && modelId !== defaultId) {
    flags.push('--model', modelId);
  }

  // Reasoning effort → CLI flags per agent
  const effort = getReasoningEffort(agentName);
  if (effort) {
    if (agentName === 'codex') {
      flags.push('--reasoning-effort', effort);
    }
    // claude/gemini: no standard CLI flag; effort stored for API consumers
  }
  ...
  for (const agent of orderedAgents) {
    ...
    summary[agent] = {
      ...
      presets: Object.fromEntries(
        Object.entries(agentModels).filter(([k]) => !['active', 'reasoningEffort'].includes(k))
      ),
    };
  }
  summary._mode = mode;
