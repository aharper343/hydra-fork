...
export function registerAgent(name, def) {
  ...
  const entry = {
    ...
    cli: type === AGENT_TYPE.PHYSICAL ? (def.cli || lower) : null,
...
export function bestAgentFor(taskType, opts = {}) {
  ...
  for (const [name, agent] of _registry) {
    ...
    candidates.push({ name, score: agent.taskAffinity[taskType] || 0 });
  }
...
export const MODEL_REASONING_CAPS = {
  // OpenAI o-series: reasoning.effort param, CLI --reasoning-effort
  'o1':        { type: 'effort',  levels: ['low', 'medium', 'high'], default: 'medium' },
  'o3':        { type: 'effort',  levels: ['low', 'medium', 'high'], default: 'medium' },
  'o4-mini':   { type: 'effort',  levels: ['low', 'medium', 'high'], default: 'medium' },

  // GPT-5 family: no reasoning control
  'gpt-5':     { type: 'none' },

  // Claude: extended thinking with token budget
  'claude-opus':   { type: 'thinking', levels: ['off', 'light', 'standard', 'deep'], budgets: { light: 4096, standard: 16384, deep: 65536 }, default: 'off' },
  'claude-sonnet': { type: 'thinking', levels: ['off', 'light', 'standard', 'deep'], budgets: { light: 4096, standard: 16384, deep: 65536 }, default: 'off' },
  'claude-haiku':  { type: 'none' },

  // Gemini: thinking via model variant swap
  'gemini-3-pro':  { type: 'model-swap', variants: { standard: 'gemini-3-pro-preview', deep: 'gemini-3-pro-deep-think' }, default: 'standard' },
  'gemini-3-flash': { type: 'none' },
  'gemini-2.5':    { type: 'none' },
};
...
function normalizeLegacyModelId(agentName, modelId) {
  ...
  if (agentName === 'codex' && (lower === 'codex-5.3' || lower === 'gpt-5.3')) {
...
export function getModelFlags(agentName) {
  ...
  if (effort) {
    ...
    } else if (caps.type === 'thinking') {
      // Claude: --thinking-budget <tokens> when not 'off'
      // Map legacy values: high → deep, medium → standard, low → light
      const LEGACY_MAP = { high: 'deep', xhigh: 'deep', medium: 'standard', low: 'light' };
      const normalized = LEGACY_MAP[effort] || effort;
      const budget = caps.budgets?.[normalized];
      if (budget && normalized !== 'off') {
        flags.push('--thinking-budget', String(budget));
      }
    }
