...
export async function pickAgent() {
  ...
  const items = order.map((name) => ({
    ...
    effort: getReasoningEffort(name),
  }));
...
export async function pickEffort(agentName) {
  const current = getReasoningEffort(agentName);

  const items = [
    { id: null, label: 'default', desc: 'use model default' },
    ...REASONING_EFFORTS.map((e) => ({ id: e, label: e, desc: null })),
  ];

  const initialIdx = current ? items.findIndex((i) => i.id === current) : 0;
...
async function main() {
  ...
  console.log('');
  const effortPick = await pickEffort(agentName);
  if (effortPick === null) {
    console.log(pc.dim('  Cancelled.\n'));
    process.exit(0);
  }
  ...
  const effortTag = effortLevel ? pc.yellow(effortLevel) : pc.dim('default');
  console.log(`\n  ${pc.green('✓')} ${pc.bold(agentName)} → ${pc.white(resolved)} ${pc.dim('effort:')} ${effortTag}  ${pc.dim('(mode → custom)')}\n`);
}
