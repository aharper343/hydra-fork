...
/**
 * Typewriter-style reveal: prints text character by character with a small delay.
 * Returns a promise that resolves when complete. Skippable via Ctrl+C.
 * @param {string} text - Text to reveal
 * @param {object} [opts] - Options
 * @param {number} [opts.delayMs=15] - Delay between characters in ms
 * @param {boolean} [opts.newline=true] - Add newline at end
 */
export function typewriterReveal(text, opts = {}) {
  const isTTY = process.stdout?.isTTY;
...
/**
 * Fade-in effect: briefly show text dim, then normal.
 * @param {string} text - Text to fade in
 * @param {boolean} [newline=true] - Add newline at end
 */
export function fadeIn(text, newline = true) {
  const isTTY = process.stdout?.isTTY;
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      // Step 2: overwrite with normal brightness (move up, clear, rewrite)
      if (!newline) {
        process.stdout.write('\r');
      } else {
        process.stdout.write('\x1b[1A\r\x1b[2K'); // move up, clear line
      }
      process.stdout.write(text);
      if (newline) process.stdout.write('\n');
      resolve();
 ...
 * @param {keyof SPINNER_STYLES} [opts.style='braille'] - Spinner style (braille, dots, moon, arrow, bounce, pulse, clock)
 */
export function createSpinner(message, opts = {}) {
  ...
  function render() {
    if (!isTTY) return;
    const frame = ACCENT(frames[frameIdx % frames.length]);
