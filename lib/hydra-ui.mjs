...
export function hydraSplash() {
  return [
    ...
    "",
    `  ${colorGradientLetters("H Y D R A")}  ${pc.gray("Hybrid Yielding Deliberation & Routing Automaton")}`,
    "",
  ].join("\n");
}
...
export function renderStatsDashboard(metrics, usage) {
  ...
  const header = `  ${'Agent'.padEnd(10)} ${'Calls'.padStart(6)} ${'Tokens'.padStart(11)} ${'Cost'.padStart(8)} ${'Avg Time'.padStart(9)} ${'Success'.padStart(8)}`;
  ...
  for (const [agent, data] of Object.entries(metrics.agents)) {
    ...
    const tokenStr = fmtTokens(tokenVal).padStart(11);
    ...
    const rate = data.successRate !== undefined
        ...
        : DIM('   -'.padStart(8));
    lines.push(`  ${agentLabel} ${calls} ${tokens} ${cost} ${avgTime} ${rate}`);
  }
...
/**
 * Compact progress bar for the status bar token gauge.
 * @param {number} percent - 0-100
