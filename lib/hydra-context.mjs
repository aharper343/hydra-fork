...
function detectKeyFiles(projectRoot) {
  // Try to extract from CLAUDE.md "Code Entry Points" section
  const claudeMd = readFileSafe(path.join(projectRoot, 'CLAUDE.md'));
  if (claudeMd) {
...
function buildMediumContext(projectConfig) {
  ...
  // Extract additional context from CLAUDE.md if available
  const claudeMd = readFileSafe(path.join(projectConfig.projectRoot, 'CLAUDE.md'), 200);
  if (claudeMd) {
 ...
 *
 * @param {string} agentName - 'claude', 'gemini', 'codex', or undefined (defaults to medium)
