...
/**
 * Write prompt to a temp file for piping on Windows.
 * Returns the temp file path. Caller must clean up.
 */
function writeTempPrompt(agent, prompt) {
  const tmpFile = path.join(os.tmpdir(), `hydra_prompt_${Date.now()}_${agent}.txt`);
  fs.writeFileSync(tmpFile, prompt, 'utf8');
  return tmpFile;
}

/**
 * On Windows, cmd.exe has an 8191-char command-line limit.
 * For long prompts, we write to a temp file and pipe via stdin.
 * CLIs (claude, gemini, codex) read from stdin when piped.
 */
function modelCall(agent, prompt, timeoutMs) {
  ...
  // Windows cmd.exe has 8191-char limit. Always use stdin on Windows to avoid truncation.
  const needsStdin = isWindows;

  if (agent === 'codex') {
    const outputPath = path.join(os.tmpdir(), `${runId('hydra_codex')}.md`);
    if (needsStdin) {
      // Codex: pipe via stdin
      const promptFile = writeTempPrompt(agent, prompt);
      try {
  ...
  if (needsStdin) {
    // Claude and Gemini: pipe prompt via stdin (auto-enters print mode)
...
function buildStepPrompt(step, userPrompt, transcript, round, totalRounds) {
  ...
  const jsonSchemas = {
    propose: [
      ...
      '  "recommended_tasks": [{"owner":"claude|gemini|codex|human","title":"string","rationale":"string","definition_of_done":"string"}],',
      '  "questions": [{"to":"claude|gemini|codex|human","question":"string"}],',
    ...
    implement: [
      ...
      '  "should_open_council": true|false,',
      '  "council_reason": "string",',
      '  "task_allocations": [{"owner":"claude|gemini|codex|human","title":"string","rationale":"string","definition_of_done":"string"}],',
      '  "review_chain": [{"from":"claude|gemini|codex","to":"claude|gemini|codex","purpose":"string"}],',
      '  "questions": [{"to":"claude|gemini|codex|human","question":"string"}],',
      '  "risks": ["string"],',
...
async function main() {
  ...
  const report = {
    ...
    publish,
    rounds,
    councilFlow: COUNCIL_FLOW.map((s) => `${s.agent}:${s.phase}`),
    url,
  ...
  for (let round = 1; round <= rounds; round += 1) {
    for (const step of COUNCIL_FLOW) {
      const promptText = buildStepPrompt(step, prompt, report.transcript, round, rounds);
      ...
      spinner.start();
      const result = modelCall(step.agent, promptText, timeoutMs);
  ...
  const crossAgentQuestions = report.questions.filter((q) => ['claude', 'gemini', 'codex'].includes(q.to)).length;
  ...
  console.log(label('Flow', `${colorAgent('claude')}${DIM('(propose)')} \u2192 ${colorAgent('gemini')}${DIM('(critique)')} \u2192 ${colorAgent('claude')}${DIM('(refine)')} \u2192 ${colorAgent('codex')}${DIM('(implement)')}`));
  console.log(label('Consensus', pc.white(short(report.consensus || '(none)', 180))));
  console.log(label('Tasks', pc.white(String(report.tasks.length))));
  console.log(label('Questions', pc.white(String(report.questions.length))));
  ...
  console.log(label('Rationale', DIM(report.recommendationRationale)));
